...
function ProfessorCard({ member }) {
  return (
    <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-6 sm:p-8 flex flex-col sm:flex-row gap-6 items-start hover:shadow-md transition-shadow">
      {/* Photo */}
      <div className="flex-shrink-0">
        <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-2xl overflow-hidden ring-4 ring-blue-100 shadow-sm">
          <img
            src={member.photo}
            alt={member.name}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 mt-4">
          <a
            ...
            className="inline-flex items-center gap-1.5 text-gray-600 hover:text-blue-600 text-xs font-medium transition-colors"
          >
            <Mail size={14} />
            {member.email}
          </a>
...
function StudentCard({ member }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 flex flex-col hover:shadow-md hover:border-blue-200 transition-all group">
      {/* Photo + name row */}
      <div className="flex items-center gap-4 mb-4">
        <div className="w-16 h-16 rounded-xl overflow-hidden ring-2 ring-gray-100 group-hover:ring-blue-100 transition-all flex-shrink-0">
          <img
            src={member.photo}
            alt={member.name}
            className="w-full h-full object-cover"
        ...
        <div className="min-w-0">
          ...
          <p className="text-blue-600 text-xs font-medium mt-0.5">{member.degree}</p>
          {member.year && (
...
function AlumnusRow({ member }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 flex items-start gap-4 hover:shadow-sm hover:border-blue-200 transition-all group">
      {/* Avatar */}
      <div className="w-12 h-12 rounded-xl overflow-hidden ring-2 ring-gray-100 group-hover:ring-blue-100 transition-all flex-shrink-0">
        <img
          src={member.photo}
          alt={member.name}
          className="w-full h-full object-cover"
    ...
    >
      ...
      <span
        className={`inline-flex items-center justify-center min-w-[20px] h-5 px-1 rounded-full text-xs font-bold ${
          active ? 'bg-blue-600 text-blue-100' : 'bg-gray-100 text-gray-500'
        }`}
      >
        {count}
      </span>
...
export default function Members() {
  ...
  return (
    <>
      ...
      {/* ── Join CTA ── */}
      <section className="bg-blue-800 text-white mt-8">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-14 text-center">
          <h2 className="text-2xl font-bold mb-3">Interested in Joining PHI Lab?</h2>
          <p className="text-blue-200 text-sm leading-relaxed mb-6 max-w-xl mx-auto">
