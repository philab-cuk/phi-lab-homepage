...
export default function AdminNews() {
  ...
  async function save() {
    ...
    if (!payload.title) { setError(new Error('title 필수')); return }
    ...
    try {
      if (isNew) {
        ...
        const { data, error } = await supabase.from('news').insert(payload).select().maybeSingle()
        ...
        if (!data || data.length === 0) {
          notify('저장되지 않았습니다 — 수정 권한 없음',
            permissionLines({ email: user?.email, role, subject: '이 소식', action: '수정' }))
          return
        }
      }
      closeModal(); load()
      ...
      if (isPermissionError(e)) {
        notify('저장되지 않았습니다 — 권한 없음',
          permissionLines({ email: user?.email, role, subject: '이 소식', action: isNew ? '등록' : '수정' }))
      } else {
        setError(dup ? new Error('같은 날 같은 제목·작성자의 뉴스가 이미 있습니다. 중복 저장이 차단되었어요 — 기존 글을 편집하세요.') : e)
      }
  ...
  async function del(row) {
    ...
    const { data, error } = await supabase.from('news').delete().eq('id', row.id).select('id')
    if (error) {
      if (isPermissionError(error)) {
        notify('삭제되지 않았습니다 — 권한 없음',
          permissionLines({ email: user?.email, role, subject: '이 소식', action: '삭제' }))
      } else setError(error)
      return
    }
    if (!data || data.length === 0) {
